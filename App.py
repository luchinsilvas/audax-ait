# AIT: Cálculo de Índice de Similitud de un jugador respecto al perfil
...
st.write('Algunos jugadores no poseen player_height registrado, por lo que debe seleccionar otro jugador (Ej: Yahir Salazar)')
st.divider()
...
with cont_1:
    ...
    with rc:
        cont_rc = st.container()
        rc_a , rc_b , rc_c = cont_rc.columns(3)

        with rc_a:
            rc_a.metric(label="Edad",value=cont_edad,delta=delta_edad , delta_color="inverse")
            rc_a.metric(label="Partidos",value=cont_partidos , delta=delta_partidos , delta_color="off")
            
        with rc_b:
            if cont_peso != 0:
                rc_b.metric(label="Peso (kg)",value=cont_peso , delta=delta_peso , delta_color="inverse")
                rc_b.metric(label="Titular", value= cont_otro , delta=delta_otro , delta_color="off")
            else:
                st.markdown("Peso no encontrado")
                st.markdown('<p style="color:#e8e7e3;font-size:20px">Este es un espacio para mantener el orden</p>', unsafe_allow_html=True)
                rc_b.metric(label="Titular", value= cont_otro , delta=delta_otro , delta_color="off")

        with rc_c:
            if cont_altura != 0:
                rc_c.metric(label="Altura (cm)",value=cont_altura , delta=delta_altura)
                rc_c.metric(label="Min. Jugados", value= cont_minutos , delta=delta_minutos , delta_color="off")
            else:
